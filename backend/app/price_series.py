...
SEGMENTS = ('land_residential', 'land_commercial', 'house_new', 'apartment', 'mehr')

def sale_segment(code, usage, condition):
...
def sale_series(rows, region_key='all'):
    ...
    for row in rows:
        segment = sale_segment(row['code'], row['usage_type'], row['house_condition'])
        ...
        if segment != 'mehr' and (area is None or area <= 0):
            continue
        value = int(row['sale_price_toman']) if segment == 'mehr' else float(row['sale_price_toman']) / float(area)
